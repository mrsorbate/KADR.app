...
router.get('/:id', (req, res) => {
    ...
    `).get(eventId);
        ...
        // Check membership
        const membership = init_1.default.prepare('SELECT role FROM team_members WHERE team_id = ? AND user_id = ?').get(event.team_id, req.user.id);
        if (!membership) {
            return res.status(403).json({ error: 'Not a team member' });
        }
        // Get responses
    ...
    `).all(eventId);
        ...
        if (!canViewResponses) {
            responses = responses.filter((response) => response.user_id === req.user.id);
        }
...
router.post('/', (req, res) => {
    try {
        const { team_id, title, type, description, location, location_venue, location_street, location_zip_city, pitch_type, meeting_point, arrival_minutes, start_time, end_time, rsvp_deadline, duration_minutes, visibility_all = true, invite_all = true, invited_user_ids = [], repeat_type, repeat_until, repeat_days } = req.body;
        ...
        const teamSettings = init_1.default.prepare(`SELECT default_response, default_rsvp_deadline_hours,
       ...
       FROM teams WHERE id = ?`).get(team_id);
        ...
        const defaultArrivalMinutesByType = {
        ...
        const selectedDefaultArrivalMinutes = defaultArrivalMinutesByType[type || 'other'] ?? legacyDefaultArrivalMinutes;
        ...
        const defaultRsvpDeadlineHoursByType = {
        ...
        const selectedDefaultRsvpDeadlineHours = defaultRsvpDeadlineHoursByType[type || 'other'] ?? legacyDefaultRsvpDeadlineHours;
        ...
        const members = init_1.default.prepare('SELECT user_id FROM team_members WHERE team_id = ?').all(team_id);
        ...
        if (isRecurring) {
            ...
            const stmt = init_1.default.prepare('INSERT INTO events (team_id, title, type, description, location, location_venue, location_street, location_zip_city, pitch_type, meeting_point, arrival_minutes, start_time, end_time, rsvp_deadline, duration_minutes, visibility_all, invite_all, created_by, series_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
            ...
            for (const { start, end } of eventDates) {
                const result = stmt.run(team_id, title, type, description, resolvedLocation, location_venue || null, location_street || null, location_zip_city || null, pitch_type || null, resolvedMeetingPoint, resolvedArrivalMinutes, start.toISOString(), end.toISOString(), getDefaultRsvpDeadline(start.toISOString()), duration_minutes ?? null, visibility_all ? 1 : 0, invite_all ? 1 : 0, req.user.id, seriesId);
            ...
            // Create single event
            const stmt = init_1.default.prepare('INSERT INTO events (team_id, title, type, description, location, location_venue, location_street, location_zip_city, pitch_type, meeting_point, arrival_minutes, start_time, end_time, rsvp_deadline, duration_minutes, visibility_all, invite_all, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
            const result = stmt.run(team_id, title, type, description, resolvedLocation, location_venue || null, location_street || null, location_zip_city || null, pitch_type || null, resolvedMeetingPoint, resolvedArrivalMinutes, start_time, resolvedEndTime, getDefaultRsvpDeadline(start_time), duration_minutes ?? null, visibility_all ? 1 : 0, invite_all ? 1 : 0, req.user.id);
...
router.put('/:id', (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const { title, type, description, location, location_venue, location_street, location_zip_city, pitch_type, meeting_point, arrival_minutes, start_time, end_time, rsvp_deadline, duration_minutes, visibility_all, } = req.body;
        if (!title || !type || !start_time || !end_time) {
            return res.status(400).json({ error: 'Missing required fields' });
        }
        const event = init_1.default.prepare('SELECT id, team_id FROM events WHERE id = ?').get(eventId);
        if (!event) {
            return res.status(404).json({ error: 'Event not found' });
        }
        const membership = init_1.default.prepare('SELECT role FROM team_members WHERE team_id = ? AND user_id = ?').get(event.team_id, req.user.id);
       ...
       WHERE id = ?`).run(title, type, description || null, resolvedLocation, location_venue || null, location_street || null, location_zip_city || null, pitch_type || null, meeting_point || null, arrival_minutes === null || arrival_minutes === undefined || Number.isNaN(arrival_minutes) ? null : arrival_minutes, start_time, end_time, rsvp_deadline || null, duration_minutes === null || duration_minutes === undefined || Number.isNaN(duration_minutes) ? null : duration_minutes, visibility_all === false || visibility_all === 0 ? 0 : 1, eventId);
...
router.post('/:id/response', (req, res) => {
    try {
        ...
        const event = init_1.default.prepare('SELECT team_id FROM events WHERE id = ?').get(eventId);
        if (!event) {
            return res.status(404).json({ error: 'Event not found' });
        }
        const membership = init_1.default.prepare('SELECT role FROM team_members WHERE team_id = ? AND user_id = ?').get(event.team_id, req.user.id);
        ...
        const stmt = init_1.default.prepare(`
...
router.post('/:id/response/:userId', (req, res) => {
    try {
        ...
        const targetMembership = init_1.default.prepare('SELECT id FROM team_members WHERE team_id = ? AND user_id = ?').get(event.team_id, userId);
        if (!targetMembership) {
...
router.delete('/:id', (req, res) => {
    try {
        ...
        // Check if event exists
        const event = init_1.default.prepare('SELECT team_id, series_id FROM events WHERE id = ?').get(eventId);
        if (!event) {
            return res.status(404).json({ error: 'Event not found' });
        }
        // Check if user is trainer
        ...
        if (deleteSeries && event.series_id) {
            ...
            const result = init_1.default.prepare('DELETE FROM events WHERE series_id = ?').run(event.series_id);
